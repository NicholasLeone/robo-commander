...
//     this->_base_width = 0.213;
     this->_base_width = 0.42545;
     this->_max_speed = 2.0;

     this->_qpps_per_meter = 2339;
//     this-> _qpps_per_meter = 3187;
     this->_wheel_diameter = 0.1524;
...
//     this->_ser_handles.push_back(h);
     ...
     const char *test_port = "/dev/ttyACM0";

     this-> claw = new RoboclawUnix(test_port, claw_addr, timeout, doack);
...
//    cout<<"Left Amp: "<<leftAmp<<endl;
//    cout<<"Right Amp: "<<rightAmp<<endl;
     /** Convert recevied information into user-friendly format */
	...
	if(verbose) {
         ...
         this->_motor_currents = currentVec;
         this->_lock.unlock();
    }
}
...
//    if(isnan(curPose[0])) {
//        cout<<"Cur X isNan true"<<endl;
//        curX = 0.0;
//    }
//    else {
//        curX = this->_cur_pose[0];
//    }
     if(isnan(curPose[1])) curY = 0.0;
     else curY = curPose[1];
//    if(isnan(curPose[1])) {
//        cout<<"Cur Y isNan true"<<endl;
//        curY = 0.0;
//    }
//    else {
//        curY = this->_cur_pose[1];
//    }
     if(isnan(curPose[2])) curYaw = 0.0;
     else curYaw = curPose[2];
//    if(isnan(curPose[2])) {
//        cout<<"Cur Yaw isNan true"<<endl;
//        curYaw = 0.0;
//    }
//    else {
//        curYaw = this->_cur_pose[2];
//    }

//    printf("_cur Poses [X (m), Y (m), Yaw (rad)]: %.5f | %.5f | %.5f\r\n", this->_cur_pose[0], this->_cur_pose[1], this->_cur_pose[2]);
//    cout<<"Current dTheta: "<< dTheta << endl;
//    printf("Cur Poses [X (m), Y (m), Yaw (rad)]: %.5f | %.5f | %.5f\r\n", curPose[0], curPose[1], curPose[2]);
//
//    printf("_cur_pose Mem. Add. [X (m), Y (m), Yaw (rad)]: %p | %p | %p\r\n", this->_cur_pose, this->_cur_pose + 1, this->_cur_pose + 2);




     float dX, dY, dYaw;
//    printf("Deltas Mem. Add. [dX (m), dY (m), dYaw (rad)]: %p | %p | %p\r\n", &dX, &dY, &dYaw);
     if(isnan(dTheta)){
          dTheta = 0.0;
          dX = linDistTraveled * cos(curYaw);
          dY = linDistTraveled * sin(curYaw);
          dYaw = 0.0;
//         printf("Current Deltas [dX (m), dY (m), dYaw (rad)]: %.5f | %.5f | %.5f\r\n", dX, dY, dYaw);
     } else{
//          float r = linDistTraveled / dTheta;
//          dX = r * (sin(curYaw + dTheta) - sin(curYaw));
//          dY = -r * (cos(curYaw + dTheta) - cos(curYaw));
          dX = linDistTraveled * cos(curYaw + (dTheta / 2.0));
          dY = linDistTraveled * sin(curYaw + (dTheta / 2.0));
...
//         printf("Current Deltas [dX (m), dY (m), dYaw (rad)]: %.5f | %.5f | %.5f\r\n", dX, dY, dYaw);
     ...
     if(isnan(dY)) dY = 0.0;
//     if(isnan(dX)) {
//         cout<<"dX isNan true"<<endl;
//         dX = 0.0;
//     }
//     if(isnan(dY)) {
//         cout<<"dY isNan true"<<endl;
//         dY = 0.0;
//     }
...
//           printf("Current Pose [X (m), Y (m), Yaw (rad)]: %.3f     |    %.3f   |       %.3f\r\n",_current_pose[0],_current_pose[1],_current_pose[5]);
    ...
    this->_lock.lock();
//    printf("Final Pose [X (m), Y (m), Yaw (rad)]: %.5f | %.5f | %.5f\r\n", curX + dX, curY + dY, curYaw + dYaw);
    ...
    this->_cur_pose[2] = curYaw + dYaw;
     ...
     this->_odom_changes[3] = dYaw;
//     printf("Updated [X (m), Y (m), Yaw (rad)]: %.5f | %.5f | %.5f\r\n", updatedX, updatedY, updatedYaw);
//     printf("Actual Pose #3 [X (m), Y (m), Yaw (rad)]: %.5f | %.5f | %.5f\r\n", this->_cur_pose[0], this->_cur_pose[1], this->_cur_pose[2]);
     this->_lock.unlock();
}
