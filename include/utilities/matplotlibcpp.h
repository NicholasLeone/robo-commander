...
void imshow(const cv::Mat &image, const std::map<std::string, std::string> &keywords = {})
{
    ...
    // if(keywords.empty()) detail::imshow(image2.data, npy_type, image2.rows, image2.cols, image2.channels(), keywords);
    // else detail::imshow(image2.data, npy_type, image2.rows, image2.cols, image2.channels(), keywords, nullptr);
...
inline long figure(std::string title = "")
{
    detail::_interpreter::get();
    long number = 0;
    if(title.empty()) number = -1;
